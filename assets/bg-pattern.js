...
// SVG version of the provided geometric ribbon pattern.
// Stack: glow(z0) < pattern(z1) < content(z2)
// - NO requestAnimationFrame (static)
// - Default color: pure white (#fff) and fully opaque
// - You can override color/opacity via CSS variables:
//   :root { --bg-pattern-color:#fff; --bg-pattern-opacity:1; }
...
(function () {
  try {
    ...
    function run() {
      ...
      // PATTERN DATA
      // ==========================
      // Tile size (px). This is the repeat unit extracted from the provided image.
      var TILE_W = 480;
      var TILE_H = 576;

      // Vector path data (filled shapes) for the linework.
      // Fill color is "currentColor" (=> controlled via svg.style.color / --bg-pattern-color).
      // NOTE: This is intentionally static (no param tuning) to match the attached image pattern.
      ...
      // Build SVG once (DOM API; avoids SVG innerHTML namespace pitfalls)
      function buildOnce() {
        // clear existing
        ...
        pattern.setAttribute("width", String(TILE_W));
        pattern.setAttribute("height", String(TILE_H));
        ...
        rect.setAttribute("id", "bg-pattern-rect");
      ...
      function resize() {
        ...
        // Keep 1 user unit == 1 CSS px
        svg.setAttribute("viewBox", "0 0 " + w + " " + h);
        svg.setAttribute("width", String(w));
        svg.setAttribute("height", String(h));

        var rect = svg.querySelector("#bg-pattern-rect");
        if (rect) {
      ...
      // init
      if (!svg.querySelector("defs")) buildOnce();
      resize();
