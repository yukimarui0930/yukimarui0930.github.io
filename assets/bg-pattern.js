...
(function () {
  try {
    ...
    function run() {
      ...
      // Start strong; you can dial back later.
      ...
      // (Your current cleaned path is used as-is; composition stays the same.)
      ...
      // 1) feMorphology(dilate) thickens slightly and eats tiny trace dents
      // 2) feGaussianBlur smooths steps
      // IMPORTANT: This produces anti-aliased edge (partial alpha) around the hole.
      // If you truly force 0/1 only, jaggies come back. This is the practical sweet spot.
      var filt = createEl(
        "filter",
        {
      ...
      // Seam fix: draw 3x3 shifted copies so tile boundaries always have continuity.
      ...
      // 3x3 offsets
      var offsets = [
        [-TILE_W, -TILE_H], [0, -TILE_H], [TILE_W, -TILE_H],
        [-TILE_W, 0],       [0, 0],       [TILE_W, 0],
        [-TILE_W, TILE_H],  [0, TILE_H],  [TILE_W, TILE_H],
      ];
      ...
      // White = fully opaque
      createEl("rect", { x: 0, y: 0, width: "100%", height: "100%", fill: "#fff" }, mask);
      // Black pattern = transparent cutouts
      createEl("rect", { x: 0, y: 0, width: "100%", height: "100%", fill: "url(#bgCutPattern)" }, mask);
